		...
		public override void HandleMcpeCraftingData(McpeCraftingData message)
		{
			...
			foreach (Recipe recipe in message.recipes)
			{
				...
				if (shapelessRecipe != null)
				{
					...
					{
						writer.WriteLine($"new Item({itemStack.LegacyId}, {itemStack.Metadata}, {itemStack.Count}){{ UniqueId = {itemStack.UniqueId}, RuntimeId={itemStack.BlockRuntimeId} }},");
					}
					writer.Indent--;
					writer.WriteLine($"}},");
				...
				if (shapedRecipe != null)
				{
					...
					{
						writer.WriteLine($"new Item({item.LegacyId}, {item.Metadata}, {item.Count}){{ UniqueId = {item.UniqueId}, RuntimeId={item.BlockRuntimeId} }},");
					}
					writer.Indent--;
					writer.WriteLine($"}},");
				...
				{
					writer.WriteLine($"new SmeltingRecipe(new Item({smeltingRecipe.Result.LegacyId}, {smeltingRecipe.Result.Metadata}, {smeltingRecipe.Result.Count}){{ UniqueId = {smeltingRecipe.Result.UniqueId}, RuntimeId={smeltingRecipe.Result.BlockRuntimeId} }}, new Item({smeltingRecipe.Input.LegacyId}, {smeltingRecipe.Input.Metadata}){{ UniqueId = {smeltingRecipe.Input.UniqueId}, RuntimeId={smeltingRecipe.Input.BlockRuntimeId} }}, \"{smeltingRecipe.Block}\"),");
					continue;
				}
