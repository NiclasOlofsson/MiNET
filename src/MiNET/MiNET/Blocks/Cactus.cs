...
namespace MiNET.Blocks
{
	public class Cactus : Block
	{
		...
		public override void OnTick(Level level, bool isRandom)
		{
			...
			if (twoBlockDowns.Id != Id && level.GetBlock(up).Id != Id
			) //If 2 blocks beneath us && the block above us is not sugar cane, then do growing.
			{
				...
				else if (isRandom && twoBlockDowns.Id != Id && Metadata < 15
				) //Do not do the ageing when we are already 3 blocks tall or when we are already at growth 15
				{
		...
		{
			return ItemFactory.GetItem(351, 2); //Cactus green dye
		}
	}
}