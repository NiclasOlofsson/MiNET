...
namespace MiNET
{
	public class PlayerNetworkSession
	{
		...
		private void HandlePackage(IMcpeMessageHandler handler, Package message)
		{
			...
			{
				HandleMcpeLogin((McpeLogin) message);
			}
		...
		public virtual void HandleMcpeLogin(McpeLogin message)
		{
			...
			lock (_loginSyncLock)
			{
				...
				{
					Log.InfoFormat("Player {0} doing multiple logins", Player.Username);
			...
			{
				Player.Server.GreylistManager.Greylist(EndPoint.Address, 30000);
			...
			{
				{
					...
					{
						Player.Username = Player.CertificateData.ExtraData.DisplayName;
						string identity = Player.CertificateData.ExtraData.Identity;

						if (Log.IsDebugEnabled) Log.Debug($"Connecting user {Player.Username} with identity={identity}");
						Player.ClientUuid = new UUID(new Guid(identity));

						CryptoContext = new CryptoContext
						{
							UseEncryption = Config.GetProperty("UseEncryptionForAll", false) || (Config.GetProperty("UseEncryption", true) && !string.IsNullOrWhiteSpace(Player.CertificateData.ExtraData.Xuid)),
						};

						if (CryptoContext.UseEncryption)
						{
							ECDiffieHellmanPublicKey publicKey = CryptoUtils.CreateEcDiffieHellmanPublicKey(Player.CertificateData.IdentityPublicKey);
...

							{
								...
								CryptoContext.Algorithm = rijAlg;
								CryptoContext.Decryptor = decryptor;
								CryptoContext.Encryptor = encryptor;
								CryptoContext.InputStream = inputStream;
								CryptoContext.OutputStream = outputStream;
								CryptoContext.CryptoStreamIn = cryptoStreamIn;
								CryptoContext.CryptoStreamOut = cryptoStreamOut;
							}
							...
							if (Log.IsDebugEnabled) Log.Warn($"Encryption enabled for {Player.Username}");
							Player.SendPackage(response);
						}
					}
				}

				{
					...
					Player.ServerAddress = payload.ServerAddress;
					Player.ClientId = payload.ClientRandomId;
				...
				if (!CryptoContext.UseEncryption)
				{
					Player.HandleMcpeClientMagic(null);
				}
			}
			catch (Exception e)
			{
		...
		public void SendPackage(Package package)
		{
			...
			if (!isBatch)
			{
				var result = Player.Server.PluginManager.PluginPacketHandler(package, false, Player);
		...
		private void SendQueue(object state)
		{
			...
			try
			{
				...
				for (int i = 0; i < lenght; i++)
				{
					...
					if (lenght == 1)
					{
						Player.Server.SendPackage(this, package);
					}
		...
		private void SendBuffered(int messageCount)
		{
			...
			Player.Server.SendPackage(this, batch);
		}
