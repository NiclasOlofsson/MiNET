...
using MiNET.Worlds.Generators.Survival.Structures;
			...
			try
			{
				...
				if (surface && y >= OverworldGenerator.WaterLevel)
				{
					...
					if (x >= 3 && x <= 13 && z >= 3 && z <= 13)
					{
						...
						if (tree == null && biome.GenerateTrees && biome.Downfall >= 0 && (noise > ((1f - biome.Downfall) /* + 0.5*/) /*+ (y/512f)*/) && noise > (95f * biome.Downfall / (256f - y)))
						{
							if (currentTemperature >= 0.95f && biome.Downfall >= 0.2f)
							{
								if (currentTemperature > 1f && GetRandom(8) == 4)
								{
					...
					if (!generated)
					{
						if (currentTemperature <= 0.1f)
						{
							column.SetBlock(x, y + 1, z, 78); //SnowLayer
						}

						if (noise > 0.45) //Threshold 1
						{
							...
							if (currentTemperature > 0.3f && currentTemperature < 1.5f && biome.Downfall >= 0)
							{
								...
								if ( /*sugarPosibility <= 11*/ noise > 0.6f &&
								...
								else if (noise > 0.55 && biome.Downfall >= 0.3f && (blockBeneath == 3 || blockBeneath == 2)) //If above 0.8, we generate flowers :)
								{
